...
- [ ] T4.7 — Unread-message browser notification when the widget/chat tab isn't focused — two real options with different trade-offs, not yet decided: (a) lightweight, tab title + favicon badge, no permission prompt, or (b) native `Notification` API popup, needs a one-time browser permission grant, shows even when the tab isn't visible at all — or both. If (b): default to a generic "New message" body with no content preview, consistent with this project's privacy posture elsewhere (PII redaction for AI, anonymous-by-default sessions) rather than surfacing real message text in an OS notification tray.
